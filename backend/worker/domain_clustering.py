"""Operator clustering — group competitors that share tracking codes."""
...
# Confidence mapping: fingerprint type -> cluster confidence level
CONFIDENCE_MAP = {
...
def compute_clusters() -> int:
    """Recompute operator clusters from current fingerprints.
    ...
    # Group by (type, value) -> set of competitor_ids
    groups: dict[tuple[str, str], set[str]] = defaultdict(set)
    for row in rows:
        key = (row["fingerprint_type"], row["fingerprint_value"])
        groups[key].add(row["competitor_id"])

    # Filter to groups with 2+ competitors (actual sharing)
    ...
    # Delete clusters that no longer have 2+ members (code no longer shared)
